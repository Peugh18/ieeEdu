...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="p-4 md:p-12 max-w-[1440px] mx-auto space-y-6 md:space-y-12 pb-24">
            ...
            <!-- Premium Header -->
            <header class="relative overflow-hidden bg-gradient-to-br from-on-background to-[#2D302B] rounded-2xl md:rounded-[3rem] p-5 md:p-16 mb-3 md:mb-12 shadow-2xl shadow-on-background/20">
                <!-- Decor -->
                ...
                <div class="relative z-10 space-y-4 md:space-y-6 max-w-4xl">
                    <div class="inline-flex items-center gap-3 px-4 py-2 bg-white/5 backdrop-blur-xl rounded-full border border-white/10 shadow-inner">
                        ...
                        <span class="text-[9px] md:text-[10px] font-bold text-white/90 uppercase tracking-[0.25em]">Expediente Académico</span>
                    </div>
                    
                    <h1 class="text-xl md:text-6xl font-serif font-bold text-white leading-tight tracking-tight">
                    ...
                    <div class="pt-2 md:pt-6">
                        <div class="relative max-w-md group">
                            ...
                            <input 
                                ...
                                class="w-full pl-12 pr-6 py-3 md:py-4 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl text-white text-sm focus:ring-4 focus:ring-[#D4AF37]/10 focus:border-[#D4AF37]/40 transition-all placeholder:text-white/30 font-serif italic"
                            />
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col lg:flex-row justify-between items-center gap-4 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm p-3 md:p-4 rounded-2xl md:rounded-[2.5rem] border border-white dark:border-slate-800 shadow-sm w-full">
                <div class="grid grid-cols-3 gap-2 w-full lg:flex lg:flex-row lg:gap-3">
                    <button 
                        v-for="filter in filters" 
                        :key="filter.id"
                        @click="currentFilter = filter.id"
                        class="flex items-center justify-center gap-2 px-3 py-3 md:px-6 md:py-3.5 rounded-xl md:rounded-2xl text-[9px] md:text-[11px] font-black uppercase tracking-wider md:tracking-[0.15em] transition-all duration-500 group relative overflow-hidden w-full lg:w-auto"
                    ...
                    >
                        <component :is="filter.icon" class="w-3.5 h-3.5 shrink-0" :class="currentFilter === filter.id ? 'text-[#D4AF37]' : 'text-primary/40'" />
                        <span>
                ...
                <div class="flex items-center justify-between lg:justify-end gap-6 w-full lg:w-auto shrink-0">
                    <div class="text-[10px] md:text-[11px] font-bold text-on-surface-variant/40 uppercase tracking-widest whitespace-nowrap flex items-center gap-2">
            ...
            <div v-if="filteredCourses.length > 0">
                ...
                <div v-if="viewMode === 'grid'" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    <article 
                        ...
                        :key="course.id"
                        class="bg-white dark:bg-slate-900 rounded-3xl border border-outline-variant/15 dark:border-slate-800/80 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden group"
                    >
                        <!-- Visual Cover -->
                        <div class="relative aspect-video overflow-hidden bg-surface-container-highest">
                            ...
                            <div class="absolute top-4 left-4">
                                <span class="px-3 py-1.5 rounded bg-black/35 backdrop-blur-md text-[9px] font-bold tracking-widest uppercase text-white border border-white/10 shadow-sm">
                                    {{ course.type }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="p-5 md:p-6 flex flex-col flex-1 justify-between">
                            <div class="space-y-3">
                                 <div class="flex items-center gap-3 text-[9px] font-bold text-primary/70 dark:text-primary-fixed-dim/80 uppercase tracking-widest">
                                     {{ course.category?.name }}
                                 </div>
                                 <h3 class="font-serif font-bold text-lg md:text-xl text-on-background dark:text-on-surface leading-tight hover:text-primary transition-colors line-clamp-2" :title="course.title">
                                    {{ course.title }}
                                 </h3>
                            </div>

                            <div class="space-y-3.5 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800/60">
                                <!-- Progress Bar -->
                                <div class="space-y-1">
                                    <div class="flex items-center justify-between text-[10px] text-on-surface-variant/60 font-bold uppercase tracking-wider">
                                        <span class="flex items-center gap-1.5 italic">
                                            <CheckCircle v-if="course.pivot.completed_at || (course.pivot.progress || 0) >= 100" class="w-3.5 h-3.5 text-[#D4AF37]" />
                                            <PlayCircle v-else class="w-3.5 h-3.5 text-primary" />
                                            {{ (course.pivot.completed_at || (course.pivot.progress || 0) >= 100) ? 'Módulo Completado' : 'Sesiones en curso' }}
                                        </span>
                                ...
                                <Link 
                                    ...
                                    class="w-full py-3.5 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-[0.2em] flex justify-center items-center gap-2 hover:bg-on-background transition-all duration-300 shadow-sm relative group/btn overflow-hidden"
                                >
                                    <span class="relative z-10">{{ course.pivot.completed_at || (course.pivot.progress || 0) >= 100 ? 'Repasar Lecciones' : 'Aula Virtual' }}</span>
                                    <ArrowRight class="w-3.5 h-3.5 relative z-10 group-hover/btn:translate-x-1 transition-transform" />
                                </Link>
                            </div>
                        </div>
                    </article>
                    ...
                    <div class="md:hidden space-y-4">
                        ...
                        >
                            <div class="flex items-center gap-3">
                                <div class="w-16 h-10 bg-slate-50 dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700/50 rounded-lg overflow-hidden flex-shrink-0">
                                    <img v-if="course.image" :src="course.image" :alt="course.title" class="w-full h-full object-cover">
                                </div>
                                <div class="flex flex-col min-w-0">
                                    <span class="text-[9px] font-bold uppercase tracking-widest text-[#D4AF37] mb-0.5">{{ course.category?.name }}</span>
                                    <h4 class="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight line-clamp-1" :title="course.title">{{ course.title }}</h4>
                                </div>
                            </div>
                            
                            <div class="flex items-center justify-between gap-4 pt-3 border-t border-slate-100 dark:border-slate-800/60">
                                <span class="inline-flex items-center px-2.5 py-1 rounded bg-slate-50 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                    {{ course.type }}
                                </span>
                            ...
                            <div class="space-y-1.5">
                                <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 font-medium">
                                    <span>Progreso</span>
                                    <span class="font-bold text-slate-800 dark:text-slate-200">{{ course.pivot.progress || (course.pivot.completed_at ? 100 : 0) }}%</span>
                                </div>
                            ...
                            <div class="pt-2">
                                <Link 
                                    :href="route('student.classroom', { course: course.slug })"
                                    class="w-full py-3.5 rounded-xl bg-primary text-white text-[11px] font-black uppercase tracking-[0.2em] flex justify-center items-center gap-2 hover:bg-on-background transition-all duration-300"
                                >
