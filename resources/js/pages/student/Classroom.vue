...
import type { Course, Lesson, Material, QuizStats } from '@/types/classroom';
...
const {
    ...
    props.allLessonsCompleted,
    props.allLessonsCount,
);
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div
            class="flex h-[calc(100svh-4rem)] w-full max-w-full flex-col overflow-hidden bg-background text-on-background selection:bg-primary/20 selection:text-primary"
        >
            <ClassroomHeader
                :course="course"
                :current-lesson="currentLesson"
                :current-lesson-index="currentLessonIndex"
                :all-lessons-count="allLessonsCount"
                :prev-lesson-id="prevLessonId"
            ...
            />
            <main class="relative flex w-full max-w-full flex-1 flex-col overflow-hidden bg-background lg:flex-row">
                <div
                    class="custom-scrollbar relative flex-1 overflow-y-auto overflow-x-hidden border-r border-outline-variant/10 bg-white p-4 shadow-sm md:p-8"
                >
                    <ClassroomExamSection
                        v-if="viewingExam"
                        :course="course"
                        :quiz-stats="quizStats"
                        :local-all-completed="progress.localAllCompleted.value"
                        :local-completed-length="progress.localCompleted.value.length"
                        :all-lessons-count="allLessonsCount"
                    />
                    <ClassroomContent
                        v-else
                    ...
                    <button
                        @click="toggleMobileSidebar('chat')"
                        class="fixed bottom-20 right-6 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-white shadow-2xl transition-all active:scale-95 lg:hidden"
                    >
                        <MessageSquare class="h-6 w-6" />
                        <span
                            v-if="comments.length > 0"
                            class="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[9px] font-black text-white"
                            >{{ comments.length }}</span
                        >
                    </button>
                </div>
                <ClassroomSidebar
                    v-model:new-comment="commentsApi.newComment.value"
                    v-model:mobile-sidebar-open="mobileSidebarOpen"
                    v-model:replying-to="commentsApi.replyingTo.value"
                    v-model:editing-comment="commentsApi.editingComment.value"
                    :course="course"
                    :current-lesson="currentLesson"
                    :current-lesson-index="currentLessonIndex"
                    :all-lessons-count="allLessonsCount"
                    :local-completed="progress.localCompleted.value"
                    :local-all-completed="progress.localAllCompleted.value"
                    :active-sidebar-tab="activeSidebarTab"
                    :comments="comments"
                    :current-user="currentUser"
                    :quiz-stats="quizStats"
                    :time-since="commentsApi.timeSince"
                    @update:active-sidebar-tab="activeSidebarTab = $event"
                    @post-comment="commentsApi.postComment(currentLesson?.id ?? 0)"
                    @toggle-like="commentsApi.toggleLike"
                    @delete-comment="commentsApi.deleteComment"
                    @update-comment="commentsApi.updateComment"
                    @open-exam="handleOpenExam"
                />
            </main>
