            ...
            <div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-wrap gap-3 mb-12">
                    <button 
                        ...
                        :class="selectedCategory === 'Todas' ? 'bg-primary text-white' : 'bg-surface-dim text-primary hover:bg-surface-container-highest'"
                    >
                        Todos
                    </button>
                    <button 
                        ...
                        class="px-5 py-2.5 rounded-full text-sm font-bold transition-colors shadow-sm"
                        :class="selectedCategory === cat.name ? 'bg-primary text-white' : 'bg-surface-dim text-primary hover:bg-surface-container-highest'"
                    >
                        {{ cat.name }}
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12" v-if="regularMasterclasses.length > 0">
                    ...
                    >
                        ...
                        <div class="p-6 flex flex-col flex-1">
                            <div class="flex items-center gap-4 text-[13px] font-medium text-on-surface-variant mb-4">
                                <div class="flex items-center gap-1.5">
                                    <svg class="w-4 h-4 text-outline-variant" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    {{ course.start_date ? new Date(course.start_date).toLocaleDateString() : 'Por definir' }}
                                </div>
                                <div class="flex items-center gap-1.5">
                                    <svg class="w-4 h-4 text-outline-variant" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    {{ course.start_time || 'Por definir' }}
                                </div>
                            </div>

                            <Link :href="route('cursos.show', { slug: course.slug, ...(isDashboard ? { dashboard: true } : {}) })" class="mb-5 block">
                                <h3 class="font-serif font-bold text-xl text-on-background leading-tight group-hover:text-primary transition-colors" :title="course.title">
                                    {{ course.title }}
                                </h3>
                            </Link>
                            ...
                            <div class="mt-auto flex flex-col gap-2">
                                ...
                                <Link :href="route('cursos.show', { slug: course.slug, ...(isDashboard ? { dashboard: true } : {}) })" class="w-full justify-center flex items-center gap-2 rounded-full text-[10px] uppercase tracking-widest font-bold px-4 py-2 border border-outline-variant/40 text-on-surface-variant hover:bg-surface-container-highest hover:text-on-background transition-colors">
                ...
                <div v-if="featuredMasterclass" class="bg-surface-container-highest rounded-3xl overflow-hidden mt-8 mb-16 shadow-[0_20px_40px_rgba(26,28,25,0.06)] grid grid-cols-1 lg:grid-cols-2 relative">
                    <!-- Left Content -->
                    <div class="p-8 md:p-12 lg:p-14 flex flex-col justify-center">
                        <span class="inline-flex w-max rounded-full bg-tertiary-container text-on-tertiary-container px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest mb-6">
                            Masterclass Destacada
                        </span>
                        <h2 class="font-serif italic font-bold text-4xl lg:text-5xl text-on-background leading-tight mb-6">
                            {{ featuredMasterclass.title }}
                        </h2>
                        <p class="text-on-surface-variant leading-relaxed mb-8 max-w-lg">
                        ...
                        <div class="flex flex-wrap items-center gap-6 mb-10">
                            <div class="flex items-center gap-2 text-sm font-bold text-on-background">
                            ...
                            <div class="hidden sm:block w-[1px] h-6 bg-outline-variant/50"></div>
                            <div v-if="featuredMasterclass.lessons?.[0]?.start_time" class="flex items-center gap-2 text-sm font-bold text-on-background">
                        ...
                        <div class="flex flex-wrap items-center gap-4 mt-2">
                            <a :href="featuredMasterclass.lessons?.[0]?.live_link || '#'" target="_blank" class="w-full sm:w-auto justify-center inline-flex items-center gap-2 rounded-xl text-[13px] uppercase tracking-widest font-bold px-8 py-4 bg-primary text-white hover:bg-[#4a4a24] transition-all shadow-xl hover:-translate-y-0.5">
                            ...
                            <Link :href="route('cursos.show', { slug: featuredMasterclass.slug, ...(isDashboard ? { dashboard: true } : {}) })" class="w-full sm:w-auto justify-center inline-flex items-center gap-2 rounded-xl text-[13px] uppercase tracking-widest font-bold px-8 py-4 border-2 border-primary/20 text-primary hover:bg-primary/5 transition-all">
                                MÁS INFORMACIÓN
                            </Link>
                        </div>
                    </div>
                    ...
                    <div class="relative min-h-[300px] lg:min-h-[500px]">
                        ...
                        <div v-else class="absolute inset-0 w-full h-full bg-gradient-to-tr from-primary/80 to-on-background/90 rounded-r-3xl flex items-center justify-center">
