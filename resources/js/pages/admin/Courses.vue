    ...
    <AppLayout>
        ...
        <div class="mb-10 flex flex-col gap-8">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between px-2">
                <div>
                    <h1 class="font-serif text-4xl text-on-surface">Gestión de <span class="italic text-primary">Cursos</span></h1>
                ...
                <button
                    ...
                    class="inline-flex items-center justify-center gap-3 rounded-full bg-gradient-to-br from-primary to-[#6d6d35] px-8 py-3.5 text-[14px] font-bold text-white shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all w-full sm:w-auto font-sans border-none"
                >
                    <Plus class="w-5 h-5" />
                    Crear nuevo curso
                </button>
            ...
            <div class="bg-white p-5 rounded-[2.5rem] border border-outline-variant/10 shadow-sm shadow-black/5 flex flex-col xl:flex-row gap-5 items-center relative z-10 w-full">
                ...
                <div class="flex-1 w-full relative">
                    <Search class="absolute left-6 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-primary/40" />
                    <input
                        v-model="filtersForm.search"
                        placeholder="Buscar por título, ID o descripción..."
                        class="w-full bg-surface-container-highest rounded-full pl-14 pr-6 py-4 text-[14px] font-sans outline-none focus:ring-2 focus:ring-primary/20 transition-all border-transparent placeholder:text-primary/30 font-medium"
                ...
                <div class="w-full xl:w-auto flex flex-col md:flex-row gap-4">
                    ...
                    <div class="relative w-full md:w-48">
                        <select v-model="filtersForm.status" class="w-full rounded-full bg-surface-container-highest pl-6 pr-10 py-4 text-[13px] font-bold uppercase tracking-wider text-primary border-transparent outline-none focus:ring-2 focus:ring-primary/20 appearance-none cursor-pointer transition-all">
                            <option value="">Todos los Estados</option>
                            <option value="BORRADOR">Borrador</option>
                            <option value="PUBLICADO">Publicado</option>
                            <option value="OCULTO">Oculto</option>
                        </select>
                        <Filter class="absolute right-5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-primary/40 pointer-events-none" />
                    </div>

                    <!-- Type Filter -->
                    <div class="relative w-full md:w-56">
                        <select v-model="filtersForm.type" class="w-full rounded-full bg-surface-container-highest pl-6 pr-10 py-4 text-[13px] font-bold uppercase tracking-wider text-primary border-transparent outline-none focus:ring-2 focus:ring-primary/20 appearance-none cursor-pointer transition-all">
                    ...
                    <!-- Apply -->
                    <button class="rounded-full bg-primary hover:bg-[#4a4a24] px-8 py-4 text-[13px] font-bold text-white transition-all shadow-md shadow-primary/10 border-none" @click="applyFilters">
                        Filtrar
                    </button>
                </div>
            </div>
                        ...
                        >
                            ...
                            <div class="col-span-1 lg:col-span-2 flex items-center justify-center lg:justify-end">
                                ...
                                <div class="flex items-center bg-surface-container-highest/30 p-2 rounded-2xl border border-outline-variant/20 lg:border-transparent group-hover:border-outline-variant/20 group-hover:bg-white group-hover:shadow-xl transition-all duration-700 opacity-100 lg:opacity-40 group-hover:opacity-100 transform lg:group-hover:-translate-x-2">
                                    ...
                                    <!-- Duplicar -->
                                    <button @click="duplicateCourse(course)" class="p-3 rounded-xl hover:bg-primary/5 text-on-surface-variant hover:text-primary transition-all hover:scale-110" title="Clonación Instantánea">
