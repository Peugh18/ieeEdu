...
import { BookOpen, Wallet, Check } from 'lucide-vue-next';
...
<template>
    ...
    <AppLayout>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div class="hidden lg:col-span-3 lg:flex flex-col gap-2">
                    <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id"
                        ...
                        :class="activeTab === tab.id ? 'bg-primary border-primary text-white shadow-xl' : 'bg-white border-transparent text-on-surface-variant hover:bg-surface-container-low'">
                        <div class="flex items-center gap-3">
                            <component :is="tab.icon" class="w-5 h-5" :class="activeTab === tab.id ? 'text-white' : 'text-on-surface-variant/50'" />
        ...
        <Transition enter-active-class="transition duration-500" enter-from-class="translate-y-full opacity-0" enter-to-class="translate-y-0 opacity-100">
            <div v-if="flash.success" class="fixed bottom-10 right-10 z-[100] flex items-center gap-4 rounded-3xl bg-surface-container-highest p-2 pr-6 text-on-surface shadow-2xl border border-outline-variant/10">
                <div class="w-10 h-10 rounded-full bg-emerald-500 flex items-center justify-center"><Check class="h-6 w-6 text-white" /></div>
                <div class="flex flex-col">
                    <span class="text-[10px] uppercase font-bold tracking-widest text-emerald-600">Operación Exitosa</span>
