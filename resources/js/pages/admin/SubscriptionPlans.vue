...
<template>
    ...
    <AppLayout>
        <div class="mx-auto max-w-6xl space-y-8 px-4 py-8">
            <AdminPageHeader title="Planes " title-accent="Premium" subtitle="Precios, duración y beneficios visibles en /planes." compact />

            <div class="grid gap-6">
                <article v-for="plan in plans" :key="plan.id" class="overflow-hidden rounded-[2rem] border border-slate-100 bg-white shadow-sm">
                    <form @submit.prevent="savePlan(plan)" class="space-y-5 p-8">
                        ...
                        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                            <div>
                                ...
                                <input
                                    v-model="forms[plan.id].name"
                                    class="mt-1 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-bold"
                                />
                            </div>
                            <div>
                                <label class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Precio (S/)</label>
                                <input
                                    v-model.number="forms[plan.id].price"
                                    type="number"
                                    step="0.01"
                                    min="0"
                                    class="mt-1 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-bold"
                            ...
                            <div>
                                ...
                                <input
                                    v-model.number="forms[plan.id].months"
                                    type="number"
                                    min="1"
                                    class="mt-1 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-bold"
                                />
                            ...
                            <div>
                                ...
                                <input
                                    v-model="forms[plan.id].period_label"
                                    placeholder="3 meses"
                                    class="mt-1 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm"
                                />
                            </div>
                            <div>
                                <label class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Etiqueta destacada</label>
                                <input
                                    v-model="forms[plan.id].badge"
                            ...
                            <div>
                                ...
                                <input
                                    v-model.number="forms[plan.id].sort_order"
                                    type="number"
                                    min="0"
                                    class="mt-1 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-bold"
                                />
                        ...
                        <div>
                            ...
                            <textarea
                                v-model="forms[plan.id].description"
                                rows="2"
                        ...
                        <div>
                            <label class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400"
                                >¿Qué incluye? (un beneficio por línea)</label
                            >
                            <textarea
                            ...
                            ></textarea>
                            <p v-if="forms[plan.id].errors.features" class="mt-1 text-xs text-rose-500">{{ forms[plan.id].errors.features }}</p>
                        </div>
                        ...
                        <div class="flex justify-end">
                            <button
                                ...
                                class="inline-flex h-11 items-center gap-2 rounded-xl bg-primary px-6 text-xs font-bold uppercase tracking-wider text-white disabled:opacity-60"
                            >
                                <Loader2 v-if="savingId === plan.id" class="h-4 w-4 animate-spin" />
