...
<template>
    ...
    <AppLayout>
        ...
        <!-- Elite Gallery Grid -->
        <div v-if="filteredBooks.length > 0 && viewMode === 'grid'" class="grid gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            <div v-for="book in filteredBooks" :key="book.id" class="flex flex-col group bg-white rounded-[2.5rem] border border-outline-variant/20 shadow-sm hover:shadow-[0_30px_60px_rgba(87,87,42,0.12)] transition-all duration-700 overflow-hidden hover:-translate-y-3">
                ...
                <div class="relative aspect-[3/4] bg-surface-container-highest overflow-hidden border-b border-outline-variant/10">
                    ...
                    <!-- Free Badge -->
                    <div v-if="Number(book.price) === 0" class="absolute bottom-6 right-6">
                        <span class="px-4 py-2 rounded-full bg-emerald-500 text-[10px] font-black tracking-widest uppercase text-white shadow-lg">
                            Gratis
                        </span>
                    </div>
                </div>
                ...
                <div class="p-6 flex flex-col flex-1">
                    ...
                    <h3 class="font-serif text-xl font-bold text-on-background leading-snug group-hover:text-primary transition-colors italic line-clamp-2 mb-4" :title="book.title">
                        {{ book.title }}
                    </h3>
                    ...
                    <div class="flex items-center justify-between border-t border-surface-container-highest pt-4 mt-auto">
                        <div class="flex flex-col">
                            ...
                            <span class="text-base font-serif font-black text-on-surface">
                                {{ Number(book.price) > 0 ? `S/ ${fmt(book.price)}` : 'Acceso Gratuito' }}
                            </span>
                        </div>
                        ...
                        <div class="flex items-center bg-surface-container-highest/30 p-1.5 rounded-xl border border-outline-variant/20 lg:border-transparent group-hover:border-outline-variant/20 group-hover:bg-white group-hover:shadow-lg transition-all duration-700">
                            <AdminIconButton variant="outline" @click="openEdit(book)" size="sm" title="Editar">
                                <template #default="{ iconClass }"><Edit2 :class="iconClass" /></template>
                            </AdminIconButton>
                            <AdminIconButton variant="danger" @click="destroy(book.id)" size="sm" title="Eliminar">
        ...
        <div v-else-if="filteredBooks.length > 0 && viewMode === 'list'" class="rounded-[3rem] border border-outline-variant/10 bg-white overflow-hidden shadow-2xl shadow-surface-tint/5 relative z-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div class="overflow-x-auto custom-scrollbar">
                <table class="w-full text-left border-collapse min-w-[900px]">
                    ...
                    <tbody class="divide-y divide-outline-variant/5">
                        <tr v-for="book in filteredBooks" :key="book.id" class="group transition-all hover:bg-primary/[0.02] duration-500">
                            ...
                            <td class="px-10 py-7 text-center">
                                <div class="flex justify-center">
                                    <div class="flex items-center gap-3 px-5 py-2.5 rounded-2xl bg-white border border-outline-variant/10 shadow-sm w-fit group-hover:shadow-md transition-all duration-500"
                                        :class="Number(book.price) === 0 ? 'border-emerald-500/20 shadow-emerald-500/[0.03]' : 'border-amber-500/20 shadow-amber-500/[0.03]'"
                                    >
                                        <div class="relative flex h-2 w-2">
                                            <span v-if="Number(book.price) === 0" class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                                            <span class="relative inline-flex rounded-full h-2 w-2" :class="Number(book.price) === 0 ? 'bg-emerald-500' : 'bg-amber-400'"></span>
                                        </div>
                                        <span class="text-[11px] font-black uppercase tracking-[0.15em] mt-[0.5px]" :class="Number(book.price) === 0 ? 'text-emerald-700' : 'text-amber-700'">
