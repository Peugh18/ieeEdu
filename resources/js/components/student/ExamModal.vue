...
<template>
    <div v-if="show" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
        
        <!-- Main Exam Container -->
        <div v-if="!examResult" class="bg-white w-full max-w-4xl h-[90vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            <!-- Header -->
            <div class="px-8 py-6 bg-surface-container-lowest border-b border-outline-variant/30 flex justify-between items-center shrink-0">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center">
                        <Clock class="w-6 h-6 text-primary" :class="timeRemaining < 60 ? 'animate-pulse text-red-600' : ''" />
                    </div>
                    <div>
                        <h2 class="text-xl font-serif font-bold italic leading-none">{{ quiz.title }}</h2>
                        <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mt-1">Evaluación Final de Certificación</p>
                    </div>
                </div>

                <div class="flex items-center gap-6">
                    <div class="flex flex-col items-end">
                        <span class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">Tiempo Restante</span>
                        <span class="text-2xl font-mono font-bold tracking-tighter tabular-nums" :class="timeRemaining < 60 ? 'text-red-600' : 'text-primary'">
                            {{ formattedTime }}
                        </span>
                    </div>
                    <button @click="$emit('close')" class="p-3 hover:bg-surface-container rounded-2xl transition-colors">
            ...
            <!-- Question Content -->
            <div class="flex-1 overflow-y-auto p-8 md:p-12 custom-scrollbar bg-surface-container-lowest">
                <div v-if="quiz.questions.length > 0" class="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-300" :key="currentQuestionIndex">
                    <div class="space-y-4">
                        <span class="text-xs font-bold text-primary uppercase tracking-[0.2em] italic">Pregunta {{ currentQuestionIndex + 1 }} de {{ quiz.questions.length }}</span>
                        <h3 class="text-3xl font-bold text-on-surface leading-snug font-serif italic text-pretty">
                    ...
                    <div class="grid gap-4">
                        <button
                            ...
                            class="w-full flex items-center p-6 rounded-[2rem] border-2 transition-all group text-left shadow-sm relative overflow-hidden"
                ...
                <div v-else class="h-full flex flex-col items-center justify-center text-center p-12">
                     <AlertCircle class="w-16 h-16 text-on-surface-variant/20 mb-4" />
                     <p class="text-xl font-serif italic text-on-surface-variant">Este examen aún no tiene preguntas configuradas.</p>
                </div>
            </div>

            <!-- Footer Navigation -->
            <div class="px-8 py-6 bg-surface-container-low border-t border-outline-variant/30 shrink-0 flex items-center justify-between">
                <button 
                    @click="prevQuestion"
                    :disabled="currentQuestionIndex === 0"
                    class="flex items-center gap-2 px-6 py-3 text-on-surface-variant font-bold text-xs uppercase tracking-widest hover:text-primary transition-all disabled:opacity-30 ripple"
                >
                ...
                <div class="flex items-center gap-4">
                    <button 
                        ...
                        @click="nextQuestion"
                        class="px-8 py-4 bg-primary text-on-primary font-bold text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/20 hover:shadow-primary/30 transition-all flex items-center gap-3 active:scale-95"
                    >
                        Siguiente Pregunta <ChevronRight class="w-4 h-4" />
                    </button>

                    <button 
                        v-else-if="quiz.questions.length > 0"
                        @click="showConfirmModal = true"
                        class="px-10 py-4 bg-emerald-600 text-white font-bold text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-emerald-900/10 hover:bg-emerald-700 transition-all active:scale-95 flex items-center gap-3"
                    >
                        Finalizar Evaluación <ArrowRight class="w-4 h-4" />
                    </button>
                </div>
            </div>
        </div>
        ...
        <!-- RESULTS VIEW -->
        <div v-else class="bg-white rounded-[4rem] shadow-2xl max-w-xl w-full p-12 text-center border border-outline-variant/10 animate-in zoom-in-95 duration-500 relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <h3 class="text-4xl font-serif font-bold italic text-on-surface mb-2 leading-tight">
                    {{ examResult.status === 'aprobado' ? '¡Excelente Trabajo!' : 'Sigue intentándolo' }}
                </h3>
                <p class="text-lg font-serif italic text-on-surface-variant mb-10 uppercase tracking-widest font-bold">Nota Final: <span class="text-primary text-3xl">{{ examResult.score }}</span>/20</p>
                
                <p class="text-xl font-serif italic text-on-surface-variant mb-12 leading-relaxed">
                ...
                <div class="flex flex-col gap-4">
                    <template v-if="examResult.status === 'aprobado'">
                        <a 
                            ...
                            target="_blank"
                            class="w-full py-5 bg-emerald-600 text-white font-bold rounded-2xl shadow-xl shadow-emerald-900/20 hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-xs"
                        >
                            <Download class="w-4 h-4" /> Descargar Certificado
                        </a>
                        <button 
                            @click="$emit('close')" 
                            class="w-full py-5 bg-primary text-white font-bold rounded-2xl shadow-xl shadow-primary/20 hover:opacity-90 transition-all uppercase tracking-widest text-xs"
                        >
                            Volver al curso
                        </button>
                    </template>
                    <template v-else>
                        <button 
                            ...
                            class="w-full py-5 bg-primary text-white font-bold rounded-2xl shadow-xl shadow-primary/20 hover:opacity-90 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-xs"
                        >
        ...
        <div v-if="showConfirmModal" class="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div class="bg-white rounded-[3rem] shadow-2xl max-w-sm w-full p-10 text-center border border-outline-variant/10 animate-in zoom-in-95 duration-200">
                ...
                <h3 class="text-2xl font-serif font-bold italic text-on-surface mb-3">¿Confirmar entrega?</h3>
                <p class="text-on-surface-variant leading-relaxed font-serif italic text-base mb-8">
                ...
                <div class="flex flex-col gap-3">
                    <button 
                        ...
                        class="w-full py-4 bg-primary text-white font-bold rounded-2xl shadow-xl shadow-primary/20 hover:opacity-90 transition-all uppercase tracking-widest text-[10px]"
                    >
                        {{ isSubmitting ? 'Calificando...' : 'Sí, calificar ahora' }}
                    </button>
                    <button 
                        @click="showConfirmModal = false"
                        class="w-full py-4 text-on-surface-variant hover:text-primary font-bold rounded-2xl transition-all uppercase tracking-widest text-[10px]"
                    >
