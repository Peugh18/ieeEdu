        ...
        <div class="relative z-10 flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
            <div class="space-y-3">
                <div class="flex flex-wrap items-center gap-3">
                    <div class="inline-flex min-h-[44px] items-center gap-2 rounded-full border border-primary/10 bg-primary/5 px-4 py-1.5">
                        <span class="h-2 w-2 animate-pulse rounded-full bg-primary"></span>
                        <span class="text-xs font-bold uppercase tracking-[0.2em] text-primary">Instituto de Economía y Empresa</span>
                    </div>
                    <div
                        v-if="user.has_subscription"
                        class="inline-flex min-h-[44px] items-center gap-2 rounded-full bg-gradient-to-r from-amber-400 to-amber-600 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.2em] text-white shadow-sm"
                    >
                ...
                <div>
                    <p class="mb-1 text-sm font-medium text-on-surface-variant/80">{{ greeting }},</p>
                ...
                <div class="flex flex-wrap items-center gap-4 pt-1">
                    <div class="flex min-h-[44px] items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs uppercase tracking-widest text-on-surface-variant/80">Cursos activos</p>
                        </div>
                    </div>
                    <div class="flex min-h-[44px] items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs uppercase tracking-widest text-on-surface-variant/80">Completados</p>
                        </div>
                    </div>
                    <div class="flex min-h-[44px] items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs uppercase tracking-widest text-on-surface-variant/80">Certificados</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-shrink-0 lg:w-72">
                ...
                >
                    <div class="flex items-center justify-between">
                        <div class="flex min-h-[44px] items-center gap-2">
                            ...
                            <span class="text-xs font-bold uppercase tracking-[0.2em] text-red-600">En Vivo</span>
                        </div>
                        <div class="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg bg-red-100">
                            <Radio class="h-4 w-4 text-red-600" />
                        </div>
                    </div>
                    <div>
                        <p class="mb-0.5 text-xs text-red-600/80">{{ nextLiveClass.course_title }}</p>
                        <p class="text-base font-bold leading-snug text-red-900">{{ nextLiveClass.title }}</p>
                    </div>
                    <button
                        class="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-white shadow-sm transition-all hover:bg-red-700 active:scale-95"
                    >
                ...
                >
                    <div class="flex items-center justify-between">
                        <div class="flex min-h-[44px] items-center gap-2">
                            ...
                            <span class="text-xs font-bold uppercase tracking-[0.1em] text-on-surface-variant/80">Próxima Sesión</span>
                        </div>
                        <div class="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg bg-primary/10">
                            <Wifi class="h-4 w-4 text-primary" />
                        </div>
                    </div>
                    <div>
                        <p class="mb-0.5 text-xs text-on-surface-variant/80">{{ nextLiveClass.course_title }}</p>
                        <p class="text-sm font-bold leading-snug text-on-surface">{{ nextLiveClass.title }}</p>
                    </div>
                    <div class="grid grid-cols-4 gap-1.5 text-center">
                        <div class="flex flex-col justify-center rounded-lg bg-surface-container px-1 py-2 min-h-[44px]">
                            <p class="font-mono text-lg font-bold leading-none text-on-surface">{{ String(countdown.days).padStart(2, '0') }}</p>
                            <p class="mt-1 text-[10px] uppercase text-on-surface-variant/80">Días</p>
                        </div>
                        <div class="flex flex-col justify-center rounded-lg bg-surface-container px-1 py-2 min-h-[44px]">
                            <p class="font-mono text-lg font-bold leading-none text-on-surface">{{ String(countdown.hours).padStart(2, '0') }}</p>
                            <p class="mt-1 text-[10px] uppercase text-on-surface-variant/80">Hrs</p>
                        </div>
                        <div class="flex flex-col justify-center rounded-lg bg-surface-container px-1 py-2 min-h-[44px]">
                            <p class="font-mono text-lg font-bold leading-none text-primary">{{ String(countdown.minutes).padStart(2, '0') }}</p>
                            <p class="mt-1 text-[10px] uppercase text-on-surface-variant/80">Min</p>
                        </div>
                        <div class="flex flex-col justify-center rounded-lg bg-surface-container px-1 py-2 min-h-[44px]">
                            <p class="font-mono text-lg font-bold leading-none text-primary">{{ String(countdown.seconds).padStart(2, '0') }}</p>
                            <p class="mt-1 text-[10px] uppercase text-on-surface-variant/80">Seg</p>
                        </div>
                    </div>
                    <button
                        class="flex min-h-[44px] w-full items-center justify-center gap-1.5 rounded-xl border border-primary/20 bg-primary/10 px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-primary transition-all hover:bg-primary/20 active:scale-95"
                    >
                ...
                <div v-else class="space-y-3 rounded-2xl border border-dashed border-outline-variant/30 bg-background p-6 text-center shadow-sm">
                    <div class="inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-2xl bg-surface-container p-4">
                        <Calendar class="h-8 w-8 text-on-surface-variant/60" />
                    </div>
                    <div>
                        <p class="text-xs font-bold uppercase tracking-widest text-on-surface-variant/80">Sesiones en Vivo</p>
                        <p class="mt-1 text-xs text-on-surface-variant/70">No tienes clases próximas programadas.</p>
                    </div>
                    <Link
                        :href="route('student.live-classes.index')"
                        class="inline-flex min-h-[44px] items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-primary transition-colors hover:text-primary/80"
                    >
                        Ver calendario <ChevronRight class="h-4 w-4" />
                    </Link>
                </div>
            </div>
        </div>
