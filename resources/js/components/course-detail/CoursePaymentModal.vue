            ...
            <div v-if="show" class="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4" @click.self="emit('close')">
                <div class="bg-white rounded-[2.5rem] p-8 md:p-10 w-full max-w-lg shadow-2xl relative">
                    ...
                    <p class="text-sm text-slate-500 mb-6">
                        Estás a punto de registrar tu pago para el curso <strong>{{ course.title }}</strong> por un monto de <strong>S/ {{ Number(amount).toFixed(2) }}</strong>.
                    </p>
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        ...
                        <div v-if="form.errors.course_id" class="text-rose-600 text-xs font-bold bg-rose-50 p-3 rounded-xl border border-rose-100">
