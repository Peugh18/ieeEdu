...
const grabadoFeatures = [
    ...
    },
    {
        ...
        desc: 'Acreditación válida expedida por el IEE.',
...
const vivoFeatures = [
    {
        ...
        desc: 'Interacción directa con el docente especialista.',
    ...
    {
        title: 'Asesoría Docente',
        desc: 'Resolución de casos y consultas en tiempo real.',
    ...
    },
    {
        ...
        desc: 'Networking exclusivo con otros profesionales.',
...
const eventoFeatures = [
    {
        ...
        desc: 'Análisis de alto nivel en un encuentro único.',
...
<template>
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
        <div
            v-for="f in features"
            :key="f.title"
            class="flex flex-col items-center rounded bg-white p-6 text-center shadow-[0_20px_40px_rgba(26,28,25,0.04)]"
        >
            <svg class="mb-4 h-8 w-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="f.icon"></svg>
            <h4 class="mb-2 text-sm font-bold uppercase tracking-[0.05em] text-on-background">{{ f.title }}</h4>
            <p class="text-[13px] leading-relaxed text-on-surface-variant">{{ f.desc }}</p>
        </div>
    </div>
</template>
