            ...
            >
                <div class="relative w-full max-w-lg rounded-[2.5rem] bg-white p-8 shadow-2xl md:p-10">
                    ...
                    <p class="mb-6 text-sm text-slate-500">
                        Estás a punto de registrar tu pago para el libro <strong>{{ book.title }}</strong> por un monto de
                        <strong>S/ {{ Number(book.price).toFixed(2) }}</strong
                        >.
                    </p>
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        ...
                        <div v-if="form.errors.book_id" class="rounded-xl border border-rose-100 bg-rose-50 p-3 text-xs font-bold text-rose-600">
                            {{ form.errors.book_id }}
                        </div>
