...
it('lists free course enrollments on student courses page', function () {
    $user = User::factory()->create(['email_verified_at' => now()]);
    $course = Course::factory()->create(['price' => 0, 'sale_price' => 0, 'status' => 'PUBLICADO']);
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->has('courses', 1)
            ->where('courses.0.slug', $course->slug)
        );
});
